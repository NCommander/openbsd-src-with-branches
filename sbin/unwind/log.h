...
#define	log_warn(...)			uw_log_warn(__VA_ARGS__)
#define	log_info(...)			uw_log_info(__VA_ARGS__)

void	uw_log_init(int, int);
int	log_getdebug(void);
void	log_procinit(const char *);
void	log_setverbose(int);
int	log_getverbose(void);
void	uw_log_warn(const char *, ...)
	    __attribute__((__format__ (printf, 1, 2)));
void	log_warnx(const char *, ...)
	    __attribute__((__format__ (printf, 1, 2)));
void	uw_log_info(const char *, ...)
	    __attribute__((__format__ (printf, 1, 2)));
